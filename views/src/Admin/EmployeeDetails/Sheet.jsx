...
//         );
//         setRows(response.data.sort((a, b) => a.timeSlot.localeCompare(b.timeSlot))); // Sort rows based on timeSlot
//       } catch (error) {
//         setRows([]);
...
//         );
//         setDynamicRows(extraResponse.data.sort((a, b) => a.timeSlot.localeCompare(b.timeSlot))); // Sort dynamicRows based on timeSlot
//       } catch (error) {
//         setDynamicRows([]);
...
//     if (id) {
//       try {
//         const response = await apiurl.get(`/getEmployeesSheet/${id}`);
...
//            <button
//               onClick={() =>{ downloadAllEmployeeHit(); downloadEmployeeSheet()}}
//               className="px-8 py-3 BR rounded-lg text-primary  bg-black mt-2 md:mb-5 mb  cursor-pointer mx-6 md:mx-0"
//             >
//               Download Sheet
//             </button>
//             <span className="flex md:justify-end md:gap-5 md:items-center w-full mx-6 md:mx-0 ">
//               <span className="gradient md:px-3 py-3 w-full md:w-36 text-center  text-primary  bg-black rounded-lg cursor-pointer">
//                 {date}
//               </span>
//               <span className="text-primary w-full md:w-28 cursor-pointer md:border md:px-3 py-2 rounded-lg hover:bg-primary hover:text-black" onClick={handlePrevDate}>
//                 &#11160; prev date
//               </span>
...
//             <span className="flex justify-between md:w-full w-[130%] sm:w-full md:gap-0 gap-9  md:items-center text-white font-medium text-[22px] mt-14  mx-6 md:mx-0">
//               <p>Time</p>
//               <p className="md:ml-12 ml-5">Client</p>
//               <p>Activity</p>
//               <p className="md:mr-5">Fill Time</p>
//             </span>
//           </div>
...
//                 <span className="flex justify-between items-center text-center text-white font-extralight text-[15px] mt-14 ">
...
//             ))
//           ) : (
//             <p className="text-center text-white">No slots yet filled</p>
//           )}
...
//           ))
//         ) : (
//           <p className="text-center mb-9 mt-12 text-white">No extra work</p>
//         )}
//       </div>
//       </span>
//     </span>
  ...
  const fetchActivities = async () => {
    if (id) {
      ...
      try {
        ...
        setRows(response.data.sort((a, b) => a.timeSlot.localeCompare(b.timeSlot)));
      } catch (error) {
        setRows([]);
        console.error("Error fetching tasks:", error);
      }
      ...
      try {
        ...
        setDynamicRows(extraResponse.data.sort((a, b) => a.timeSlot.localeCompare(b.timeSlot)));
      } catch (error) {
        setDynamicRows([]);
        console.error("Error fetching extra tasks:", error);
      }
